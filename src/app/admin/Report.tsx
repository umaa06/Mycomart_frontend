  ...
  return (
    <div className="flex min-h-screen bg-[#F0E6D9]" style={{ fontFamily: 'Inter, sans-serif' }}>
      <aside className="w-64 bg-[#D2C8B5] p-6 shadow-xl">
        ...
        <nav>
          <ul className="space-y-4">
            ...
            <li>
              <button className="block w-full text-left px-4 py-3 rounded-lg text-red-600 hover:bg-red-100 hover:text-red-700 transition-all duration-200" onClick={handleLogout}>
                Logout
              </button>
            </li>
          </ul>
      ...
      <div className="flex-1 p-10 relative">
        {/* Removed the background image URL as it's not a local file in this environment */}
        ...
        <div className="relative z-10">
          ...
          <div className="bg-gray-100 p-4 rounded-lg mb-6 shadow-sm">
            <p className="text-sm font-medium text-gray-700">Your User ID: <span className="font-mono text-gray-900">{userId || 'Not Logged In'}</span></p>
          </div>
                      ...
                      {availableOrders.map((order) => (
                        <tr key={order.id} className="bg-white even:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <button onClick={() => handleAcceptOrder(order.id)} className="bg-green-600 text-white font-semibold py-2 px-4 rounded-lg shadow-md hover:bg-green-700 transition-colors duration-200">
                              Accept Order
                            </button>
                          </td>
                        </tr>
